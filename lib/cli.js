...
function start(program, callback) {
    return function() {
        ...
        if (typeof opts.persistence.factory === 'string') {
            opts.persistence.factory = persistence[opts.persistence.factory];
        }
        ...
        var setupAuthorizer = function(cb) {
            ...
            loadAuthorizer(program.creds, function(err, authorizer) {
                if (err) {
                    callback(err);
                    return;
                }
...
module.exports = function cli(argv, callback) {
    ...
    program
        ...
        .option("--creds <file>", "the file containing the credentials", "./creds.json")
    ...
    var loadAuthorizerAndSave = function (cb) {
        ...
        if (!program.creds) {
            throw new Error('the credentials file does not exits');
        }
        ...
        loadAuthorizer(program.creds, function (err, authorizer) {
            ...
            cb(null, authorizer, function(err) {
                ...
                var dirname = program.creds ? path.dirname(program.creds) : null;
                if (dirname && !fs.existsSync(dirname)) {
                    mkdirp.sync(dirname);
                }
                fs.writeFile(program.creds, JSON.stringify(authorizer.data(), null, 2), callback);
            });
        });
    };
